...
#plt.show()
p=[.00002,.05,.05,200,.00007,.01]

wvpack = lambda x, *p: np.exp(p[0]*(-1.*(x-p[3])**2)-p[4]*x)*(p[1]*np.cos(p[2]*(x-p[3])))*special.erf(p[5]*x)
...
wvpack_pout = lambda x: np.exp(popt[0]*(-1.*(x-popt[3])**2)-popt[4]*x)*(popt[1]*np.cos(popt[2]*(x-popt[3])))*special.erf(popt[5]*x)
