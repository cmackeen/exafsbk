...
#plt.show()
p=[.00002,.05,.05,200,.00007,.01,.1,.01,.1,.01]

wvpack = lambda x, *p: np.exp(p[0]*(-1.*(x-p[3])**2)-p[4]*x)*(p[1]*np.cos(p[2]*(x-p[3]))+p[6]*np.cos(p[7]*(x-p[3]))+p[8]*np.cos(p[9]*(x-p[3])))*special.erf(p[5]*x)

po=[.00002,.05,.05,200,.00007,.01]

xrang=np.linspace(0,2000,200000)


popt, pcov = optimize.curve_fit(f=wvpack, xdata=sf4['e']-13052,ydata=sf4['re'], p0=p,maxfev=20000)

wvpack_out = lambda x: np.exp(po[0]*(-1.*(x-po[3])**2)-po[4]*x)*(po[1]*np.cos(po[2]*(x-po[3])))
wvpack_pout = lambda x: np.exp(popt[0]*(-1.*(x-popt[3])**2)-popt[4]*x)*(popt[1]*np.cos(popt[2]*(x-popt[3]))+popt[6]*np.cos(popt[7]*(x-popt[3]))+popt[8]*np.cos(popt[9]*(x-popt[3])))*special.erf(popt[5]*x)
